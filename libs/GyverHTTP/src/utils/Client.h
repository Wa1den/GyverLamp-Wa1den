...
class Client : public Print {
   ...
   public:
    ...
    // отправить запрос
    bool request(const Text& path, const Text& method, const Text& headers, const FormData& data) {
        return request(path, method, headers, (uint8_t*)data.str(), data.length(), true);
    }
