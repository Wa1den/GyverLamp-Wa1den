...
class FavoritesManager
{
  public:
    ...
    static void ReadFavoritesFromStorage()
    {
      Interval = (uint16_t)db[kk::fav_interval];
      Dispersion = (uint16_t)db[kk::fav_dispersion];
      UseSavedFavoritesRunning = (uint8_t)db[kk::fav_use_saved];
      FavoritesRunning = UseSavedFavoritesRunning > 0 ? (uint8_t)db[kk::fav_running] : FavoritesRunning;

      gdb::Entry favModesEntry = db.get(kk::fav_modes_blob);
    ...
    static void SaveFavoritesToStorage()
    {
      db.set(kk::fav_running, FavoritesRunning);
      db.set(kk::fav_interval, Interval);
      db.set(kk::fav_dispersion, Dispersion);
      db.set(kk::fav_use_saved, UseSavedFavoritesRunning);
      db.set(kk::fav_modes_blob, gdb::AnyType((const void*)FavoriteModes, MODE_AMOUNT));
    }
