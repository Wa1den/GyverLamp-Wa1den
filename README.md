...
точки доступа — на `http://192.168.4.1`. Со страницы доступны питание, выбор эффекта
из нумерованного списка, ползунки яркости, скорости и масштаба, все режимы и таймеры,
перечисленные ниже, а также настройки WiFi, MQTT и NTP-сервера. Открытая страница
обновляет свои виджеты, если лампой управляют другим способом.
...
| GyverDB, GyverHTTP, StringUtils, GTL, BSON, Stamp, Table, StreamIO, FOR_MACRO (GyverLibs) | из папки `libs` | зависимости Settings и GyverDB. В GyverHTTP внесена правка на одну строку (`#ifndef` вокруг таймаута клиента), позволяющая прошивке сократить время блокировки лампы синхронным вебсервером. С неизменённой версией лампа подвисает при открытии страницы настроек |
| WiFiConnector (GyverLibs) | свежая | подключение к WiFi |
| GyverButton (GyverLibs) | свежая | кнопка |
| WebSockets (Links2004) | свежая | обновление виджетов открытой страницы |
...
показаны на странице настроек в разделе MQTT. Адрес брокера, порт, логин и пароль задаются
...
в инфо-панели (строка Firmware) и в группе «Служебное». Схема нумерации — `МАЖОР.МИНОР.ПАТЧ`:
